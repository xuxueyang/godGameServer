    ...
    class FriendManager:AbsOneHandler
    {
        public void addFriend(UserToken token, FriendManagerDTO dto)
        {
            ...
            if(roleBiz.isHasSameNameRole(dto.friendName,model.area))
            {
                ...
                write(token, MainRoomProtocol.FRIEND_ADD_SRES, RETURN_CODE.SUCCESS);
            }
            else
            {
                write(token, MainRoomProtocol.FRIEND_ADD_SRES, RETURN_CODE.MAIN_ROOM_FRIEND_ADD_ROLE_NOT_EXIST);
            }
        ...
        public void deleteFriend(UserToken token, FriendManagerDTO dto)
        {
            ...
            if(model.friendSet.Contains(dto.friendName))
            {
                ...
                write(token, MainRoomProtocol.FRIEND_DELETE_SRES, RETURN_CODE.SUCCESS);
            }
            else
            {
                write(token, MainRoomProtocol.FRIEND_DELETE_SRES, RETURN_CODE.MAIN_ROOM_DRIEND_DELETE_ROLE_NOT_EXIST);
            }
